...
import { Group, Planification, Teacher } from '../../models/planification.model';
...
export class MyGroups implements OnInit {
  ...
  downloadCalendar(): void {
    ...
    const url = `http://localhost:8888/planification-service/api/calendar/student/${this.currentUser.id_user}/pdf`;
  ...
  loadData(): void {
    ...
    // Load groups, users, and schedules
    forkJoin({
      ...
      next: (data) => {
        ...
        if (this.myGroup) {
          ...
          if (this.myGroup.teacherId) {
            this.teacher = data.users.find(u => u.id_user === this.myGroup!.teacherId) || null;
          }

          // Get group schedules
          this.schedules = data.schedules.filter(s => s.groupId === this.myGroup!.id);
        }
