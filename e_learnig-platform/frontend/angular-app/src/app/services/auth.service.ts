...
export class AuthService {
  ...
  login(request: LoginRequest): Observable<TokenResponse> {
    const fallbackUsername = this.getUsernameFromEmail(request.email);
    // Try backend lookup first, then fallback to email prefix if gateway/user-service is down.
    return this.http.get<User>(`${this.apiUrl}/user-by-email?email=${request.email}`).pipe(
      ...
      catchError(error => {
        console.warn('User-service unavailable, fallback to email prefix username:', error);
        return of(fallbackUsername);
      }),
      switchMap(username => {
        return this.loginWithKeycloak(username, request.password);
      }),
      tap(response => {
  ...
  private loginWithKeycloak(username: string, password: string): Observable<TokenResponse> {
  ...
  private getUsernameFromEmail(email: string): string {
    ...
    if (atIndex > 0) return safeEmail.substring(0, atIndex);
    return safeEmail;
  }
