...
public class SubmissionService {
    ...
    public SubmissionResponse submit(SubmissionRequest request) {
        // gameId=0 is reserved for Crossword, gameId=-1 for Word Ladder (no DB game entry)
        boolean isCrossword = request.getGameId() != null && request.getGameId() == 0;
        boolean isWordLadder = request.getGameId() != null && request.getGameId() == -1;
        boolean isSpecialGame = isCrossword || isWordLadder;
        ...
        List<Question> questions = isSpecialGame
        ...
        if (isSpecialGame) {
            // Crossword / Word Ladder: use override values from request
    ...
    private SubmissionResponse buildResponse(Submission s, Map<Long, QuestionResultDTO> results) {
        ...
        String title = gameRepository.findById(s.getGameId())
            .map(g -> g.getTitle())
            .orElse(s.getGameId() == 0 ? "Crossword" : s.getGameId() == -1 ? "Word Ladder" : "Game #" + s.getGameId());
